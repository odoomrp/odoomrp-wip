...
class SaleOrder(orm.Model):
    ...
    _inherit = 'sale.order'
    _columns = {
                'sale_variant_lines': fields.one2many('sale.variant.lines', 'sale_id', 'Sale Variant Lines'),
                }
...
class SaleVariantLines(orm.Model):
    ...
    _columns = {
                ...
                'attribute_lines': fields.one2many('attribute.value.lines', 'sale_variant_line_id', 'Attributes'),
                'final_value_lines': fields.one2many('final.values', 'sale_variant_line_id', 'Values'),
                }
    ...
    def _create_variants(self, cr, uid, ids, context=None):
        ...
        static_values = [attr_line.value_id and attr_line.value_id.id or False for attr_line in sale_variant.attribute_lines]
    ...
    def create_lines(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            for product_id, product_qty in product_ids:
                #product_id_change()??
                order = sale_obj.read(cr, uid, line.sale_id.id, ['pricelist_id', 'partner_id', 'date_order', 'fiscal_position'], context=context)
                values = sale_line_obj.product_id_change(cr, uid, [], order['pricelist_id'][0], product_id,
                    qty=product_qty,
                ...
                values['value'].update({'order_id': line.sale_id.id, 'product_id': product_id, 'product_uom_qty':product_qty})
    ...
    def bring_tmpl_attributes(self, cr, uid, ids, context=None):
        ...
        tmpl = tmpl_obj.browse(cr, uid, sale_variant.tmpl_id.id, context=context)
        for line in tmpl.attribute_line_ids:
            if line.id != tmpl.final.id:
                attribute_ids.append((0,0,{'attribute_id':line.attribute_id.id}))
            else:
                value_ids = map (lambda x: (0,0,{'value_id':int(x)}), line.value_ids)
        self.write(cr, uid, ids,{'attribute_lines':attribute_ids, 'final_value_lines':value_ids}, context=context)
        return True
