...
class ProductAttributeLine(models.Model):
    ...
    def _get_possible_predecessors(self):
        ...
        self.possible_predecessors = attrs.sorted()
    ...
    def check_predecessors_values(self):
        if self.predecessors_constraint - self.possible_predecessor_values:
            raise exceptions.Warning("There are values in constraint values "
                                     "that don't belong anymore to the "
                                     "possible ones")
    ...
    predecessors = fields.Many2many(
        ...
        domain="[('id', 'in', possible_predecessors[0][2])]")
    possible_predecessors = fields.Many2many(
        comodel_name='product.attribute',
        relation='attribute_possible_predecessors',
        column1='attribute_line',
        column2='product_attribute',
        compute='_get_possible_predecessors', readonly=True)
...
class ValueConstraint(models.Model):
    ...
    allowed_values = fields.Many2many(
        comodel_name='product.attribute.value',
        relation='possible_predecessors_rel',
        column1='value_constraint_id',
        column2='attribute_value_id',
        )
