...
class L10nEsTesoreriaFacturas(orm.Model):
    ...
    _columns = {
        ...
        'total': fields.float('Total',
                              digits_compute=dp.get_precision('Account')),
        'pendiente': fields.float('Pendiente',
                                  digits_compute=dp.get_precision('Account')),
    }
...
class L10nEsTesoreria(orm.Model):
    ...
    def _check_filtro(self, cr, uid, ids, context=None):
        for teso in self.browse(cr, uid, ids, context=context):
            if not teso.check_draft and not teso.check_proforma and \
                    not teso.check_open:
                return False
        return True
    ...
    _columns = {
        ...
        'plantilla': fields.many2one('l10n.es.tesoreria.plantilla',
                                     'Plantilla', required=True),
        ...
        'saldo_inicial': fields.float(
            'Saldo Inicial', digits_compute=dp.get_precision('Account')),
        'saldo_final': fields.function(
            _calcular_saldo, method=True, digits_compute=dp.get_precision(
                'Account'), string='Saldo Final'),
        'check_draft': fields.boolean('Borrador'),
        'check_proforma': fields.boolean('Proforma'),
        'check_open': fields.boolean('Abierto'),
        'facturas_emit': fields.many2many('l10n.es.tesoreria.facturas',
                                          'l10n_es_teso_fact_e_rel', 'teso_id',
                                          'fact_e_id', 'Facturas Emitidas'),
        'facturas_rec': fields.many2many('l10n.es.tesoreria.facturas',
                                         'l10n_es_teso_fact_r_rel', 'teso_id',
                                         'fact_r_id', 'Facturas Recibidas'),
        'pagos_period': fields.one2many('l10n.es.tesoreria.pagos.period',
                                        'tesoreria_id', 'Pagos Periodicos'),
        'pagos_var': fields.one2many('l10n.es.tesoreria.pagos.var',
                                     'tesoreria_id', 'Pagos Variables'),
        'desglose_saldo': fields.one2many('l10n.es.tesoreria.saldos',
                                          'tesoreria_id', 'Desglose de Saldo'),
    }
    ...
    def restart(self, cr, uid, ids, context=None):
        ...
        for teso in self.browse(cr, uid, ids, context=context):
            ...
            t_factura_obj.unlink(cr, uid, facturas, context=context)
            for pagoP in teso.pagos_period:
                pagoP_obj.unlink(cr, uid, pagoP.id, context=context)
            for pagoV in teso.pagos_var:
                pagoV_obj.unlink(cr, uid, pagoV.id, context=context)
        return True
        ...
        for teso in self.browse(cr, uid, ids, context=context):
            ...
            for fact_emit in teso.facturas_emit:
                ...
                if saldo_id:
                    ...
                    saldos_obj.write(cr, uid, saldo.id,
                                     {'saldo': saldo.saldo + fact_emit.total},
                                     context=context)
                else:
            ...
            for fact_rec in teso.facturas_rec:
                saldo_id = saldos_obj.search(
                    cr, uid, [('name', '=', pname),
                              ('tesoreria_id', '=', teso.id)], context=context)
                if saldo_id:
                    saldo = saldos_obj.browse(cr, uid, saldo_id[0],
                                              context=context)
                    saldos_obj.write(cr, uid, saldo.id,
                                     {'saldo': saldo.saldo - fact_rec.total},
                                     context=context)
                else:
                    saldos_obj.create(cr, uid,
                                      {'name': pname,
    ...
    def button_calculate(self, cr, uid, ids, context=None):
        ...
        for teso in self.browse(cr, uid, ids, context=context):
            ...
            if teso.check_open:
                estado.append("open")
            invoices = invoice_obj.search(cr, uid, [('date_due', '>',
                                                     teso.inicio_validez),
                                                    ('date_due', '<',
                                                     teso.fin_validez),
                                                    ('state', 'in',
            ...
            for invoice in invoice_obj.browse(cr, uid, invoices,
                ...
                id = t_factura_obj.create(cr, uid, values, context=context)
                if invoice.type == "out_invoice":
                    facturas_emit.append(id)
                elif invoice.type == "in_invoice":
                    facturas_rec.append(id)
            self.write(cr, uid, teso.id, {'facturas_emit': [(6, 0,
                                                             facturas_emit)],
                                          'facturas_rec': [(6, 0,
                                                            facturas_rec)]},
                       context=context)
            for pagoP in teso.plantilla.pagos_period:
                if pagoP.fecha > teso.inicio_validez and \
                        pagoP.fecha < teso.fin_validez and not pagoP.pagado:
                    values = {
                        'name': pagoP.name,
                        'fecha': pagoP.fecha,
            ...
            for pagoV in teso.plantilla.pagos_var:
                if pagoV.fecha > teso.inicio_validez and \
                        pagoV.fecha < teso.fin_validez and not pagoV.pagado:
                    values = {
                        'name': pagoV.name,
                        'fecha': pagoV.fecha,
