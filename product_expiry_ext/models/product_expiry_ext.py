...
class StockProductioLot(orm.Model):
    ...
    def _get_product_state(self, cr, uid, ids, field_name, args, context=None):
        ...
        for lot in self.browse(cr, uid, ids, context=context):
            alert_date, removal_date, use_date, life_date = self._get_dates(cr, uid, lot, context=context)
            res[lot.id] = 'normal'
            if life_date and life_date < today:
                res[lot.id] = 'expired'
                continue
            if alert_date and removal_date and today > alert_date and today <= removal_date:
                res[lot.id] = 'alert'
                continue
            if removal_date and use_date and today > removal_date and today <= use_date:
                res[lot.id] = 'to_remove'
                continue
            if use_date and life_date and today > use_date and today <= life_date:
    ...
    _constraints = [(_check_dates, 'Dates must be: Alert Date < Removal Date < Best Before Date < Expiry Date', ['date_alert', 'date_removal', 'date_use','date_life'])]
