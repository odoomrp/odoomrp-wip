...
class WizImportProcurementFromPlan(models.TransientModel):
    ...
    procurements = fields.One2many(
        comodel_name='wiz.import.procurement.from.plan.line',
    ...
    def import_procurements(self):
        ...
        cond = [('id', 'in', proc_ids)]
        self.env['procurement.order'].search(cond).write(
            {'plan': self.env.context['active_id']})
        plan = self.env['procurement.plan'].browse(
            self.env.context['active_id'])
...
class WizImportProcurementFromPlanLine(models.TransientModel):

    _name = 'wiz.import.procurement.from.plan.line'

    wizard_id = fields.Many2one(
        'wiz.import.procurement.from.plan', string="Wizard")
