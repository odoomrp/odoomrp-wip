...
class MrpProduction(models.Model):
        ...
        if self.state != 'draft':
            if self.product_lines:
                journal = self.env.ref(
                    'mrp_production_project_estimated_cost.analytic_journal_es'
                    'timated_materials', False)
            ...
            if self.workcenter_lines:
                journal = self.env.ref(
                    'mrp_production_project_estimated_cost.analytic_journal_es'
                    'timated_machines', False)
                for line in self.workcenter_lines:
                    ...
                    if line.hour and line.workcenter_id.product_id:
                        ...
                        vals = self._cath_information_estimated_cost(
                            ...
                            line.hour)
                        analytic_line_obj.create(vals)
                    if line.routing_wc_line.op_wc_lines:
                        for op_wc_line in line.routing_wc_line.op_wc_lines:
                            if (op_wc_line.workcenter.id ==
                                ...
                                journal = self.env.ref(
                                    'mrp_production_project_estimated_cost.ana'
                                    'lytic_journal_estimated_operators', False)
                                name = (self.name + '-' +
                                        line.routing_wc_line.operation.code
    ...
    def _cath_information_estimated_cost(self, journal, name, product_id, qty):
        ...
        vals = {'name': name,
                ...
                'amount': 0,
                'estim_standard_cost': 0,
                'estim_average_cost': 0,
                'last_purchase_cost': product_id.last_purchase_price,
                'last_sale_price': product_id.last_sale_price
                }
        if general_account:
